...
#ifndef UNISTREAM_MODULE_HPP_
#define UNISTREAM_MODULE_HPP_

/**
 * @file unistream_module.hpp
 *
...
#include "private/unistream_module_pri.hpp"
...
class IModuleObserver {
 public:
   ...
   */
  virtual void Notify(std::shared_ptr<UNIFrameInfo> data) = 0;
  /**
...
class Module : private NonCopyable {
 public:
   ...
   */
  virtual int Process(std::shared_ptr<UNIFrameInfo> data) = 0;
   ...
   */
  bool TransmitData(std::shared_ptr<UNIFrameInfo> data);
   ...
   * UNIStream Inspect tool uses this class to detect parameters of each module.
   */
 ...
 protected:  // NOLINT
   ...
   */
  int DoProcess(std::shared_ptr<UNIFrameInfo> data);
 ...
 private:    // NOLINT
  ...
  void NotifyObserver(std::shared_ptr<UNIFrameInfo> data) {
    RwLockReadGuard guard(observer_lock_);
    if (observer_) {
      observer_->Notify(data);
    }
  }
  int DoTransmitData(std::shared_ptr<UNIFrameInfo> data);
