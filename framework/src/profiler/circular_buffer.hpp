...
#ifndef UNISTREAM_FRAMEWORK_PROFILER_CIRCULAR_BUFFER_HPP_
#define UNISTREAM_FRAMEWORK_PROFILER_CIRCULAR_BUFFER_HPP_
...
inline char CircularBuffer<T>::AcquireForWrite(std::atomic_char* patom) {
  ...
  for (bool done = false; !done;) {
    prev = atom;
    if (prev != kBusy) {
      done = atom.compare_exchange_strong(prev, kBusy, std::memory_order_acquire);
    }
  }
  return prev;
}
...
template <typename T>
class CircularBuffer<T>::iterator : public std::iterator<std::random_access_iterator_tag, T, int64_t> {
 public:
  explicit iterator(const CircularBuffer* buffer, size_t index) : buffer_(buffer), index_(index) {}
...
#endif  // UNISTREAM_FRAMEWORK_PROFILER_CIRCULAR_BUFFER_HPP_
